...
async function analyze() {
  ...
  try {
    ...
    const response = await fetch('./api/prompt', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ image, output_format: $('#outputFormat').value, preferred_aspect: $('#aspect').value, provider: $('#promptProvider').value, notes: $('#notes').value }) });
...
function markReady() { const metadata = makeMetadata(); const extension = $('#outputFormat').value === 'PNG' ? 'png' : 'jpg'; state.finalFileName = `${slug($('#metaTitle').value)}-final.${extension}`; $('#readyAssetName').textContent = $('#metaTitle').value.trim() || 'Final master'; $('#readyAssetInfo').textContent = `${metadata.output_format} · ${state.upscale ? `${state.upscale.provider} · ${state.upscale.model} · ${formatDimensions(state.upscale.final_width, state.upscale.final_height)}` : 'Manual upload · upscale not verified'}`; const assetLink = $('#downloadAsset'); assetLink.href = state.finalUrl || state.generatedUrl; assetLink.download = state.finalFileName; $('#downloadMetadata').onclick = () => downloadBlob(new Blob([JSON.stringify(metadata, null, 2)], { type: 'application/json' }), `${slug(metadata.title)}-metadata.json`); setStatus('READY UPLOAD'); go('ready'); }
